...
4. **Overflow — la carte ne grandit ni ne scrolle JAMAIS** : A/R sont réduits à 2 lignes (`--clamp`, `-webkit-line-clamp`). Un **« Voir plus ↗ »** (injecté par JS **seulement** si le texte déborde, mesuré) ouvre le **texte complet dans un Modal** (titre = la dimension, corps = texte complet) — pas de déplié inline, pas de scroll dans la carte, pas de reflow. Vérifié : carte 529px → 529px à l'ouverture, page zéro scroll. Le popover flottant et le scroll interne par champ ont été **écartés** (non premium / affordance illisible) au profit du Modal (composant DS réutilisé).
5. **Nature** : composant métier. Classes `.ds-smart*` dans `smart.css` + `smart.js` (mesure du débord → injection du bouton, ouverture/fermeture du Modal avec focus géré). Réutilise **carte définie + Button subtle + Modal**. Tokens `--smart-*` (padding = `--card-padding`, head-gap, row-padding, col-gap, letter-width/size/line/weight/color, text-color + variantes lead/fact/muted, dimension-gap, clamp-lines, more-gap/icon-gap/icon-size/color/color-hover). Monochrome, token-pur.
6. **Accessibilité** : `<ol>` (les 5 dimensions), Modal `role="dialog"` + `aria-modal` + `aria-labelledby`, fermeture croix / clic backdrop / Échap, focus renvoyé sur le déclencheur à la fermeture. Contraste muted vérifié > AA.
7. **Preview** : `preview/smart.html` (+ `.css` + `.js` + `modal.css`). Contenu de démo = l'objectif « 1000 clients mensuels ». Le Modal s'ouvre depuis « Voir plus » d'Atteignable/Réaliste.
