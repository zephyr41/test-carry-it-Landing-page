...
2. **Une seule structure, deux contextes** (convergé Nils 2026-07-05, réf = son mockup — monochrome, aligné) :
   - **Structure unique** `.ds-kpi-card` — layout `flex column` :
     `head` (eyebrow `type-data-label` à gauche [+ `.ds-kpi-card__head-actions` à droite en vue jalon : bouton « Ajouter » `ds-button--subtle ds-button--xs` puis crayon d'édition `.ds-row-action` 16px]) → **titre** (`type-h3`, 18/600 blanc) → `.ds-kpi-card__metric` = **valeur hero** (`51`, `type-data-value` 36/800 blanc) + **cible en gris** (`.ds-kpi-card__target`, `/ 15 h`, `type-body-lg` 16/400 muted) [+ **delta pill** `ds-badge` à droite sur l'effort] → **fraîcheur** (`.ds-kpi-card__meta`, `type-body-sm`) → **Progress bar** dessous. **Fraîcheur SOUS la valeur** dans les deux vues (unifié suite au /critique — cohérence) : la valeur ressort juste après le titre.
   ...
   - **Deux variantes = même structure, densité différente** : **Vue jalon (interactive)** = padding **standard système** `--kpi-card-padding` → `--card-padding` (24, PAS une valeur bespoke), header actions + Progress bar. **Vue long terme (résumé, lecture seule)** `.ds-kpi-card--summary` = **tuile étroite** : **~2x moins large** que le jalon (grille `.kpi-card-grid--compact` = colonnes fixes `--kpi-card-summary-min-width` **280px**, alignées à gauche), padding **16**, rythme **8**, **titre `type-body-md` medium BLANC** (`--color-text-primary` — tranche sur les métadonnées grises, sinon tous les gris se confondent), **valeur réduite** **28** (vs 36), pas de delta, **sans** header-actions **ni** barre. NB : c'est la **largeur** qui distingue le résumé (tuile dense d'une liste), pas un tassement vertical (interlignes gardés confortables). Ordre (unifié jalon + résumé après /critique) : eyebrow → titre → **valeur** → fraîcheur (le jalon ajoute la barre tout en bas). Rationale : la valeur doit ressortir immédiatement après le titre, pas être enterrée sous la fraîcheur. Ordre des cartes : **Effort puis Résultat**.
   - **Hauteurs uniformes (grille régulière)** : les textes à longueur variable (titre résumé, copy « à définir ») **réservent 2 lignes** (`min-height: 2lh`) → toutes les cartes d'une rangée ont la MÊME hauteur quel que soit le contenu (résumé 280×162 identiques ; cartes « à définir » alignées, boutons au même Y). Pas de `align-items:stretch` (qui créerait un void en bas) : on réserve la hauteur du texte à la source.
   ...
   - **Hiérarchie typo (rôle → classe DÉFINIE, aucun override ad-hoc)** : diagnostic mesuré (Nils 2026-07-05) — « tout les textes se ressemblent » venait d'un mauvais usage des classes (titre `type-body-lg` forcé en 600, cible `type-h3` 18/600 muted → **cible plus grosse que le titre**, même poids). Fix = chaque rôle mappé à sa classe : **valeur** `type-data-value` (36/800 blanc, héros) > **titre** `type-h3` (18/600 blanc) > **cible** `type-body-lg` (16/400 muted) > **fraîcheur** `type-body-sm` (13/400 muted) > **eyebrow** `type-data-label` (11/700 caps muted). Rampe nette, chaque texte distinct. 2 pôles blanc / muted.
   - **Différenciation effort/résultat** : par le **libellé eyebrow** + la **pill de statut** (effort dépassé → `+36 H au-delà` ; résultat sous seuil → `10 restants`), PAS par une icône de type ni par une couleur (essayées puis retirées — Nils : trop d'infos / carte monochrome).
...
- **Pill de statut (`ds-badge`, `text-transform:none`, poussée à droite de la valeur)** — symétrique effort/résultat (correctif /critique v2 : le résultat, la vraie preuve, ne doit pas être plus silencieux que l'effort) : **effort au-dessus du seuil** → `+36 H au-delà` (`+36` = `valeur − cible`, PAS le `+36H/15H` cryptique) ; **résultat sous le seuil** → `10 restants` (`cible − valeur`). Format lisible, monochrome. *Rationale anti-vanity* : on met en avant le franchissement du seuil / le restant vers la preuve, pas l'activité brute.
- **Barre en dépassement** = **barre PLEINE `--100` + tick vertical au seuil** (`.ds-kpi-card__bar-tick--N`, N% = `cible/valeur` = 29% ici ; tick sombre `--color-background` visible sur le remplissage blanc). Lit « rempli, seuil ici, dépassé ». Correctif /critique v2 : l'ancien fade (`overshoot-N`) se lisait « reste à faire » → l'effort dépassé paraissait moins avancé que le résultat on-track (à l'envers). Barre normale `0→cible` sinon. `aria-valuetext` porte la vraie mesure (« 51 h, seuil 15 h dépassé ») car `aria-valuenow` seul est faux en dépassement.
...
7. **Preview** : `preview/kpi-card.html` (+ `.css`). Vue jalon (effort rempli + delta pill/barre · résultat rempli · **effort à définir** · **résultat à définir**) + Vue long terme (mêmes cartes en lecture seule, sans barre).
