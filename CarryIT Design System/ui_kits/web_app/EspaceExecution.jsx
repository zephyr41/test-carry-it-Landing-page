  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {tasks.length === 0 ? (
          <div style={{ fontSize: 12.5, color: 'rgba(255,253,246,0.42)', fontStyle: 'italic', padding: '6px 4px' }}>—</div>
        ) : (
          tasks.map(t => (
            <div key={t.id} style={{
              background: '#0A0A0A',
      ...
      {status !== 'done' && (
        <button style={{
          ...
          color: 'rgba(255,253,246,0.42)', background: 'transparent',
...
const EspaceExecution = ({ jalons, activeJalonId, onSelectJalon, onValidate }) => {
  ...
  const sectionStyles = {
    ...
    overlineContext: { color: 'rgba(255,253,246,0.72)' },
    criterionText: {
...
export { EspaceExecution, MilestoneTab, TaskRow, TaskGroup, KanbanColumn, AddTaskRow, Chevron };
