...
const TaskRow = ({ task, onToggle, onDragStart }) => {
  ...
  return (
    <div style={rowStyles.row}
         draggable
         onDragStart={(e) => { e.dataTransfer.effectAllowed = 'move'; setDragging(true); onDragStart && onDragStart(task.id); }}
...
const TaskGroup = ({ status, label, tasks, onToggle, onAdd, onDragStart, onDrop, collapsed, onToggleCollapse }) => {
  ...
  return (
    <div
      ...
      onDrop={(e) => { e.preventDefault(); e.stopPropagation(); setDragOver(false); onDrop && onDrop(status); }}
      style={{
        borderRadius: 6,
        outline: dragOver ? `2px solid ${groupColors.headerColor}` : '2px solid transparent',
        transition: 'outline 100ms ease',
        background: dragOver ? `rgba(${status === 'doing' ? '238,68,8' : status === 'done' ? '20,184,166' : '255,253,246'},0.04)` : 'transparent',
      }}
    >
      <button
      ...
      <div style={{ display: collapsed ? 'none' : 'block' }}>
        {tasks.map(t => (
          <TaskRow key={t.id} task={t} onToggle={onToggle} onDragStart={onDragStart} />
        ))}
  ...
  const moveTask = (targetStatus) => {
    ...
    setTasks(prev => ({
      ...
      [activeJalon.id]: prev[activeJalon.id].map(t => {
        ...
        if (targetStatus === 'doing') return { ...t, done: false, priority: 'high' };
        if (targetStatus === 'todo') return { ...t, done: false, priority: undefined };
        return { ...t, done: true, priority: undefined };
      }),
    }));
    setDraggedId(null);
  };
        ...
        {view === 'list' ? (
          <div>
            <TaskGroup
              ...
              onDragStart={setDraggedId} onDrop={moveTask}
